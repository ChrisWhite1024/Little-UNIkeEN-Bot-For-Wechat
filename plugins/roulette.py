    ...
    def get_cmd(self, id, msg):
        ...
        if startswith_in(msg,['装弹','轮盘','决斗']):
            if self.status!='init':
               ...
               # print(self.player)
                try:
                    if msg_split[4][:6]=='[CQ:at':
                        aim_id=msg_split[4].replace('[CQ:at,qq=','',1)
        ...
        if startswith_in(msg,['拒绝决斗']):
            ...
            if self.aim_id==id:
                ...
                self.__init__(self.group_id)
                return ret
        if startswith_in(msg,['取消决斗']) and self.status=='prepare':
            if id==self.player[0]:
                self.__init__(self.group_id)
                return "取消决斗成功"
            else:
        ...
        # ongoing阶段，决斗
        if startswith_in(msg, CMD_ROULETTE_ONGOING) and self.status=='ongoing':
